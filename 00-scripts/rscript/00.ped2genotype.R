...
#Output: genotypic matrix (AC, TG/TC etc. with inds in row, mk in cols)
...
sdf <- sapply(start,function(i, dat3) ifelse(as.character(dat3[,i])==as.character(dat3[,i+1]),paste(as.character(dat3[,i])) , paste(as.character(dat3[,i]),as.character(dat3[,i+1]), sep="/") ) ,dat3 = dat3) 
