...
#modification from Q-Rougemont- Le 16-03-2017
argv<-commandArgs(TRUE)

chunkfile <-argv[1] ## RADpainter output file
mcmcfile<- argv[2]  ## finestructure mcmc file
treefile<- argv[3]  ## finestructure tree file

plotsFolder <- getwd()
plotsFolder <- paste(plotsFolder,"/02-output/", sep="")
analysisName <- "Analysis" ;  maxIndv <- 10000; maxPop<-10000

### 4) EDIT THE PATH TO YOUR COPY of FinestructureLibrary.R
source("00-scripts/rscript/FinestructureLibrary.R", chdir = TRUE) # read in the R functions, which also calls the needed packages
